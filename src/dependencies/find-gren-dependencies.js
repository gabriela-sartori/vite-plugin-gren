  ...
  return firstline(file).then(function(line) {
    return new Promise(function(resolve, reject) {
      ...
      if (matches) {
        ...
        //
        // See https://github.com/rtfeldman/node-gren-compiler/pull/36

        return resolve(path.dirname(file));
      }

      return reject(file + " is not a syntactically valid Gren module. Try running `gren make` on it manually to figure out what the problem is.");
    });
  });
}

// Returns a Promise that returns a flat list of all the Gren files the given
// Gren file depends on, based on the modules it loads via `import`.
function findAllDependencies(file, knownDependencies, sourceDirectories, knownFiles) {
  ...
  if (sourceDirectories) {
    ...
    return getBaseDir(file)
      .then(getGrenPackageSourceDirectories)
...
// Given a source directory (containing top-level Gren modules), locate the
// gren.json file that includes it and get all its source directories.
function getGrenPackageSourceDirectories(baseDir, currentDir) {
  if (typeof currentDir === "undefined") {
    baseDir = path.resolve(baseDir);
    currentDir = baseDir;
  }

  var grenPackagePath = path.join(currentDir, 'gren.json');
  if (fs.existsSync(grenPackagePath)) {
    var sourceDirectories = getSourceDirectories(grenPackagePath);
  ...
  return getGrenPackageSourceDirectories(baseDir, path.dirname(currentDir));
}
...
function getSourceDirectories(grenPackagePath) {
  try {
    var grenPackage = JSON.parse(fs.readFileSync(grenPackagePath, 'utf8'));
  } catch (e) {
    console.warn('Ignored malformed package metadata JSON file: ' + grenPackagePath);
    return [];
  }

  var sourceDirectories = grenPackage['source-directories'];
  if (!Array.isArray(sourceDirectories)) {
    console.warn('Ignored package metadata JSON file with missing/invalid source-directories: ' + grenPackagePath);
    return [];
  }

  return sourceDirectories.map(function(sourceDir) {
    return path.resolve(path.dirname(grenPackagePath), sourceDir);
  });
}
  ...
  return new Promise(function(resolve, reject) {
    ...
    readImports(file).then(function(lines){
        ...
        var newImports = _.compact(lines.map(function(line) {
          ...
          // all non-native modules are .gren
          var extension = ".gren";
          ...
          // e.g. ~/code/gren-css/src/Css/Declarations.gren
        ...
        var recursePromises = _.compact(validDependencies.map(function(dependency) {
          return path.extname(dependency) === ".gren" ?
